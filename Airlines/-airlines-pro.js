...
function airlines() {
  ...
  const userName = prompt("Introduzca su nombre de usuario.", "Usuario:");
  if (userName == null) {
    alert("¡Adiós!");
    return;
  } else {
    ...
    for (let i = 0; i < flights.length; i++) {
      ...
      avgCost = avgCost + flights[i].cost.valueOf();
    }
    console.log(
      ...
      if (access === "ADMIN") {
        ...
        switch (option) {
          case "ADD":
            ...
            if (confirm("¿Desea realizar otra operación como ADMIN?")) {
              airlinesPro(access);
            } else {
              break;
            }
        ...
        n++;
        const userPrice = parseInt(prompt("Introduzca el precio que busca."));
        if (isNaN(userPrice)) {
          ...
          findPrice(userPrice);
          buyFlight();
          if (confirm("¿Desea realizar otra operación como USER?")) {
            airlinesPro(access);
          }
        }
      } else {
...
airlines();
confirm("¿Desea continuar realizando operaciones?")
  ? airlines()
  : alert("¡Hasta la próxima!");

//Función para crear vuelos.
function addFlight() {
  let newFlight = { id: 0, to: " ", from: " ", cost: 0, scale: null };
  let n = 0;
...
function delFlight() {
  const id = parseInt(prompt("Introduzca el número de vuelo a eliminar."));
  let delId;
  delId = findId(id);
  if (delId == false) {
